...
private:
    ...
    {
        if (fwd.type != EMPTY && pos == fwd.rd)
            return fwd.temp_result;
...
public:
    ...
    void run(Register *reg, forward &fwd) // check forwarding in EXE, similar to run
    {
        ...
        // branch    //B type
        case BEQ:
            temp_resultpc = (imm - 4) * (getdata(reg, rs1, fwd) == getdata(reg, rs2, fwd));
            break;
        case BNE:
            temp_resultpc = (imm - 4) * (getdata(reg, rs1, fwd) != getdata(reg, rs2, fwd));
            break;
        case BLT:
            temp_resultpc = (imm - 4) * ((int)getdata(reg, rs1, fwd) < (int)getdata(reg, rs2, fwd));
            break;
        case BGE:
            temp_resultpc = (imm - 4) * ((int)getdata(reg, rs1, fwd) >= (int)getdata(reg, rs2, fwd));
            break;
        ...
        case SH:
        case SW:
            addr = getdata(reg, rs1, fwd) + imm;
            break;
        // arithmetic and logic instructions begin
        // I type
        case ADDI:
            temp_result = getdata(reg, rs1, fwd) + imm;
            break;
        case SLTI:
            temp_result = ((int)getdata(reg, rs1, fwd) < (int)imm);
            break;
        case SLTIU:
            temp_result = (getdata(reg, rs1, fwd) < imm);
            break;
        ...
        case ANDI:
            ...
            break;
        case SLLI:
            temp_result = getdata(reg, rs1, fwd) << shamt;
            break;
        case SRLI:
            temp_result = getdata(reg, rs1, fwd) >> shamt;
            break;
        case SRAI:
            temp_result = (getdata(reg, rs1, fwd) >> shamt) | (getdata(reg, rs1, fwd) >> 31 << 31);
            break;
        // the original sign bit is copied into the vacated upper bits   ???
        // B type
        case ADD:
            temp_result = getdata(reg, rs1, fwd) + getdata(reg, rs2, fwd);
            break;
        case SUB:
            temp_result = getdata(reg, rs1, fwd) - getdata(reg, rs2, fwd);
            break;
        case SLL:
            ...
            break;
        case SLT:
            temp_result = ((int)getdata(reg, rs1, fwd) < (int)getdata(reg, rs2, fwd));
            break;
        case SLTU:
            temp_result = (getdata(reg, rs1, fwd) < getdata(reg, rs2, fwd));
            break;
        ...
        case SRL:
            ...
            break;
        case SRA:
            temp_result = (getdata(reg, rs1, fwd) >> getdata(reg, rs2, fwd)) | (getdata(reg, rs1, fwd) >> 31 << 31);
            break;
        case OR:
            temp_result = getdata(reg, rs1, fwd) | getdata(reg, rs2, fwd);
            break;
    ...
    void memory_access(Memory *mem, Register *reg)
    {
        ...
        // I type
        case LB:
            temp_result = sext(mem->load(addr, 1), 7);
            break;
        case LH:
            temp_result = sext(mem->load(addr, 2), 15);
            break;
        case LW:
            temp_result = mem->load(addr, 4);
            break;
        case LBU:
            temp_result = mem->load(addr, 1);
            break;
        case LHU:
            temp_result = mem->load(addr, 2);
            break;
        // S type
        case SB:
            mem->store(addr, reg->getdata(opt.rs2), 1);
            break;
        case SH:
            mem->store(addr, reg->getdata(opt.rs2), 2);
            break;
        case SW:
            mem->store(addr, reg->getdata(opt.rs2), 4);
            break;
        }
    }
