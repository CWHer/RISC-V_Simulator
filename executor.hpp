...
public:
    ...
    void run()
    {
        ...
        case BEQ:
            temp_resultpc = opt.pc + 4 + (imm - 4) * (opt.Vj == opt.Vk);
            break;
        case BNE:
            temp_resultpc = opt.pc + 4 + (imm - 4) * (opt.Vj != opt.Vk);
            break;
        case BLT:
            temp_resultpc = opt.pc + 4 + (imm - 4) * ((int)opt.Vj < (int)opt.Vk);
            break;
        case BGE:
            temp_resultpc = opt.pc + 4 + (imm - 4) * ((int)opt.Vj >= (int)opt.Vk);
            break;
        ...
        // I type
        case ADDI:
            temp_result = opt.Vj + imm;
            break;
        case SLTI:
            temp_result = ((int)opt.Vj < (int)imm);
            break;
        case SLTIU:
            temp_result = (opt.Vj < imm);
            break;
        ...
        case ANDI:
            ...
            break;
        case SLLI:
            temp_result = opt.Vj << shamt;
            break;
        case SRLI:
            temp_result = opt.Vj >> shamt;
            break;
        case SRAI:
            temp_result = (opt.Vj >> shamt) | (opt.Vj >> 31 << 31);
            break;
        // the original sign bit is copied into the vacated upper bits   ???
        // B type
        case ADD:
            temp_result = opt.Vj + opt.Vk;
            break;
        case SUB:
            temp_result = opt.Vj - opt.Vk;
            break;
        case SLL:
            ...
            break;
        case SLT:
            temp_result = ((int)opt.Vj < (int)opt.Vk);
            break;
        case SLTU:
            temp_result = (opt.Vj < opt.Vk);
            break;
        ...
        case SRL:
            ...
            break;
        case SRA:
            temp_result = (opt.Vj >> opt.Vk) | (opt.Vj >> 31 << 31);
            break;
        case OR:
            temp_result = opt.Vj | opt.Vk;
            break;
    ...
    void memory_access(Memory *mem)
    {
        ...
        // I type
        case LB:
            temp_result = sext(mem->load(opt.A, 1), 7);
            break;
        case LH:
            temp_result = sext(mem->load(opt.A, 2), 15);
            break;
        case LW:
            temp_result = mem->load(opt.A, 4);
            break;
        case LBU:
            temp_result = mem->load(opt.A, 1);
            break;
        case LHU:
            temp_result = mem->load(opt.A, 2);
            break;
    ...
    { // pc will be modified earlier in fetch
        ...
        // store
        case SB:
            mem->store(opt.A, opt.Vk, 1);
            break;
        case SH:
            mem->store(opt.A, opt.Vk, 2);
            break;
        case SW:
            mem->store(opt.A, opt.Vk, 4);
            break;
        // branch    //B type
        //  case BEQ:;
        //  case BNE:
